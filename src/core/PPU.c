...
void ResetPalette(PPU* ppu)
{
	if (!ppu->using_default_palette)
		FreeCustomPalette(ppu);

	ppu->using_default_palette = true;
	ppu->palette = GetDefPalette();
}
...
void DrawPixelWithPal(PPU* ppu, uint8_t paletteValue, int x, int y)
{
	...
	uint8_t r = ppu->palette[3 * paletteValue + 0];
	uint8_t g = ppu->palette[3 * paletteValue + 1];
	uint8_t b = ppu->palette[3 * paletteValue + 2];
